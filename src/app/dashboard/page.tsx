...
type BalanceRes = { balance: number } | { balanceCached: string } | any;
type Bundle = { id:string; title:string; code:string; sizeMb:number; };
type Order = { orderRef:string; status:string; recipientPhone:string; amountCharged:string; createdAt:string; };
...
export default function Dashboard(){
  ...
  const load = async () => {
    setMsg("");
    try{
      // These endpoints must exist in your core-api:
      // GET /wallet/balance
      // GET /bundles?network=MTN  (or /products/bundles)
      ...
      setMsg(ex.message || "Failed to load dashboard");
    }
  };
  ...
  const onFund = async () => {
    ...
    setMsg("");
    try{
      // POST /wallet/fund/paystack/init -> { authorization_url, reference }
      const res = await api<{authorization_url:string, reference:string}>(
        "/wallet/fund/paystack/init",
        { method:"POST", auth:true, body:{ amount: fundAmount, email } }
      );
      window.location.href = res.authorization_url;
    }catch(ex:any){
      setMsg(ex.message || "Funding failed");
    }finally{
      setBusyFund(false);
    }
  };
  ...
  const onBuy = async () => {
    ...
    setMsg("");
    try{
      // POST /orders { bundleId, recipientPhone } -> { orderRef, status }
      const res = await api<{orderRef:string, status:string}>(
        "/orders",
        { method:"POST", auth:true, body:{ bundleId, recipientPhone: phone } }
      );
      setMsg(`Order created: ${res.orderRef} (${res.status})`);
      setPhone("");
      await load();
    }catch(ex:any){
      setMsg(ex.message || "Order failed");
    }finally{
      setBusyBuy(false);
    }
  };
        ...
        <div className="grid">
          <div className="card">
            ...
            <div className="row" style={{marginTop:14}}>
              <button className="btn primary" disabled={busyBuy || !bundleId || !phone}>
          ...
          <div className="card">
            ...
            <div className="kpi">
              ...
              <div className="v">GHS {balance}</div>
            </div>
            ...
            <div className="row" style={{marginTop:14}}>
              <button className="btn primary" onClick={onFund} disabled={busyFund || !email || fundAmount<=0}>
                {busyFund ? "Opening Paystack..." : "Fund wallet"}
              </button>
            ...
            <div className="notice">
              After payment, Paystack will notify the server. Refresh your dashboard to see the updated balance.
            </div>
          </div>
        </div>
      </div>
